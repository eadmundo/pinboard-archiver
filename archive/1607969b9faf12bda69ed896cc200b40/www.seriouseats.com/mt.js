function writeCommenterGreeting(commenter_name, entry_id, blog_id, commenter_id, commenter_url) {
    if ( check_permission(commenter_name, entry_id, blog_id, commenter_id, commenter_url) )
    {
        var url;
        if (commenter_id) {
            url = 'http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi?__mode=edit&return_to=' + encodeURIComponent(document.URL) + '&id=' + commenter_id + '&blog_id=' + blog_id;
        ...
        content += '. Now you can reply to this topic. (<a href="http://www.seriouseats.com/cgi-bin/mte/mt-comments.cgi?__mode=handle_sign_in&amp;static=1&amp;logout=1&entry_id=' + entry_id + '">sign out</a>)';
        document.write(content);
    } else if (commenter_name) {
            document.write('You do not have permission to comment on this blog. (<a href="http://www.seriouseats.com/cgi-bin/mte/mt-comments.cgi?__mode=handle_sign_in&amp;static=1&amp;logout=1&entry_id=' + entry_id + '">sign out</a>)');
    } else {


        document.write('<a href="http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi?__mode=login&entry_id=' + entry_id + '&blog_id=' + blog_id + '&static=1">Sign in' + '</a>' + ' to reply to this topic.');
...
(function () {
  ...
  function _initializeVariables() {
    ...
    loadingIcon = "<img title='Loading...' src='http://www.seriouseats.com/mt-static/images/indicator.white.gif' alt='Loading' />";
  ...
  function _updateComments() {
    ...
    jsonUrl = [
        "http://www.seriouseats.com/cgi-bin/mte/mt-comments.cgi?__mode=comment_listing&direction=",
        direction,
        "&entry_id=",
        entryID,
        "&limit=",
...
function mtScore(entry_id) {
    ...
    if (MT.mobile_page) {
        ...
        var url = 'http://mobile.seriouseats.com/cgi-bin/mte/mt-cp.cgi';

    } else {
        var url = 'http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi';
    }
    ...
    xh.send( '__mode=vote&blog_id=30&f=scored,count&jsonp=mtScore_cb&id=' + entry_id);
    return false;
}
...
function mtUpdateScores() {
    ...
    xh.setRequestHeader( 'Content-Type', 'application/x-www-form-urlencoded' );
    xh.send( '__mode=score&blog_id=30&f=scored,count&jsonp=mtScore_cb&id=' + entry_ids);
    return false;
}
    ...
    for (var id in s_hash) {
        ...
        if ( span ) {
            ...
            if ( s_hash[id].scored ) {
                ...
                var scoreSpan = span.getElementsByClassName('scoring-vote');
                if (scoreSpan)
                  scoreSpan[0].innerHTML = "Favorited ";
            } else {
...
function script_follow(id) {
    ...
    var xh = mtGetXmlHttp();
    if (!xh) return false;
    xh.open('POST', 'http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi', true);
    xh.onreadystatechange = function() {
        if ( xh.readyState == 4 ) {
            if (xh.status && ( xh.status != 200 ) ) {
                // error - ignore
    ...
    if (span) span.innerHTML = '<img src="http://www.seriouseats.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="Following..." />';
}
...
function script_leave(id) {
    ...
    var span = document.getElementById('following-status');
    if (span) span.innerHTML = '<img src="http://www.seriouseats.com/mt-static/images/indicator.white.gif" height="10" width="10" alt="Leaving..." />';
}
...
var mtCookieDomain = ".seriouseats.com";
var mtCookiePath = "/";
var mtCookieTimeout = 315360000;
...
function mtGetUser() {
    ...
    // alert ('returning user' + user.name);
    return user;
}
...
function mtFetchUser(cb) {
    ...
    if ( ( cb == 'mtSetUser' ) && mtGetUser() ) {
        ...
        script.src = 'http://www.seriouseats.com/cgi-bin/mte/mt-comments.cgi?__mode=session_js&blog_id=30&jsonp=' + cb + '&ts=' + ts;
...
function mtCommentOnSubmit(f) {
    if (!mtRequestSubmitted) {
        ...
        if (f.armor)
            f.armor.value = '97adf1f836eb1b8d51e70dea838f0ae24062e95a';
...
function mtSignIn() {
    ...
    var url = 'http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi?__mode=login&blog_id=30';
    
    if (MT.mobile_page) {
      	
    	var url = 'http://mobile.seriouseats.com/cgi-bin/mte/mt-cp.cgi?__mode=login&blog_id=30&tmpl_id=3291';
         // var url = 'http://mobile.seriouseats.com/user/login' + '?return_url=' + encodeURIComponent(doc_url);
    }
    ...
    if (is_preview) {
        if ( document['comments_form'] ) {
            ...
            url += '&return_url=http%3A%2F%2Fwww.seriouseats.com%2F';
        }
    } else {
...
function mtSignInOnClick(sign_in_element) {
    ...
    if (el)
		...
		if (sign_in_element == 'login-tab-inner'){
			el.innerHTML = '<div id="login-spinner"><img src="http:/static.seriouseats.com/images/siteassets/login-spinner.gif" /></div>';
		} else {
			el.innerHTML = '<div id="skybox3" class="skybox loggedIn"><div class="dropshadow2"><div class="innerbox"><h3 class="skybox3Strip">Signing in... <span class="status-indicator">&nbsp;</span></h3></div></div></div>';
		}
...
function mtSignOut(entry_id) {
    ...
    var doc_url = document.URL;
    doc_url = doc_url.replace(/#.+/, '');
    var url = 'http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi?__mode=logout&blog_id=30';
    if (is_preview) {
        if ( document['comments_form'] ) {
            var entry_id = document['comments_form'].entry_id.value;
            url += '&entry_id=' + entry_id;
...
function mtShowGreeting() {
    ...
    if ( u && u.is_authenticated ) {
        if ( u.is_banned ) {
            ...
            var user_link;
            if ( u.is_author ) {
                user_link = '<a href="http://www.seriouseats.com/cgi-bin/mte/mt-cp.cgi?__mode=edit&amp;return_to=' + encodeURIComponent(document.URL) + '&amp;blog_id=30';
                user_link += '">' + u.name + '</a>';
            } else {
